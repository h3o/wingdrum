...
static esp_err_t i2c_codec_ctrl_init(i2c_port_t set_i2c_num, int8_t enable_AGC, int8_t AGC_max_gain, int8_t AGC_target_level, int8_t mic_bias)
{
    ...
    */
    ...
    //verify if DAC and HP driver is powered up -> not implemented
    /*
    ...
    */
    ...
    //only enable ADC, leave MIC1 off
	...
	//MIC2R/LINE2R Input Level Control for Right-ADC PGA Mix
	//Setting the input level control to one of the following gains automatically connects MIC2R to the right-ADC PGA mix.
	//D3-D0: 0010: Input level control gain = �3 dB
	ret = i2c_codec_two_byte_command(0x12, 0xf2); //11110010 -> -3dB
	//ret = i2c_codec_two_byte_command(0x12, 0xf4); //11110100 -> -6dB
	//ret = i2c_codec_two_byte_command(0x12, 0xf8); //11111000 -> -12dB
	if (ret != ESP_OK) { return ret; }
...
void codec_init()
{
    ...
    if(ret == ESP_ERR_TIMEOUT) {
		printf("codec_init(): ESP_ERR_TIMEOUT (i2c)\n");

    	while(1)
    	{
    		error_blink(ERROR_BLINK_PATTERN_1357_2468, 200); //alternate pattern
    	}
    } else if(ret == ESP_OK) {
...
void codec_set_mute(int status)
{
	if(status) //mute
	{
		//only if the volume is not set to 0x80 errorneously
		if(codec_digital_volume!=CODEC_DIGITAL_VOLUME_MUTE)
		{
